...
def home(request):
    ...
    # curso do usuário (se houver)
    courses = request.user.profile.course.all()
    user_course_id = courses[0].id if courses else None
    ...
    # Une e mantém a ordenação
    all_posts = teacher_posts.union(student_posts).order_by('-created_at')
